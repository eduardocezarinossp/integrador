	...
	public void inserir(Pessoa p) {
		...
		try {
			String sql = "INSERT INTO pessoa "
					+ "( nome, email, idade,cod_med, telefone) "
					+ "VALUES (?,?,?, ?,?)";
			PreparedStatement prep = con.getConexao().prepareStatement(sql);
			prep.setString(1, p.getNome());
			prep.setString(2, p.getEmail());
			prep.setString(3, p.getIdade());
			prep.setString(4, p.getCodigoMed());
			prep.setString(5, p.getTelefone());
			prep.executeUpdate();			
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public ObservableList<Pessoa> listar() {
		...
		try {
			...
			while (res.next()) {
				...
				String med = res.getString("cod_med");
				if(med == null) med = "0";
